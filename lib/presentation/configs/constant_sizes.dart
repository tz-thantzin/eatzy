import 'package:flutter/material.dart';

const double s0 = 0.0;
const double s03 = 0.3;
const double s05 = 0.5;
const double s08 = 0.8;
const double s1 = 1.0;
const double s2 = 2.0;
const double s3 = 3.0;
const double s4 = 4.0;
const double s5 = 5.0;
const double s6 = 6.0;
const double s8 = 8.0;
const double s10 = 10.0;
const double s12 = 12.0;
const double s13 = 13.0;
const double s14 = 14.0;
const double s16 = 16.0;
const double s18 = 18.0;
const double s20 = 20.0;
const double s22 = 22.0;
const double s24 = 24.0;
const double s25 = 25.0;
const double s26 = 26.0;
const double s28 = 28.0;
const double s30 = 30.0;
const double s32 = 32.0;
const double s34 = 34.0;
const double s36 = 36.0;
const double s38 = 38.0;
const double s40 = 40.0;
const double s42 = 42.0;
const double s48 = 48.0;
const double s50 = 50.0;
const double s55 = 55.0;
const double s60 = 60.0;
const double s62 = 62.0;
const double s64 = 64.0;
const double s65 = 65.0;
const double s70 = 70.0;
const double s80 = 80.0;
const double s85 = 85.0;
const double s90 = 90.0;
const double s96 = 96.0;
const double s100 = 100.0;
const double s110 = 110.0;
const double s120 = 120.0;
const double s150 = 150.0;
const double s165 = 165.0;
const double s180 = 180.0;
const double s200 = 200.0;
const double s220 = 220.0;
const double s250 = 250.0;
const double s280 = 280.0;
const double s300 = 300.0;
const double s350 = 350.0;
const double s400 = 400.0;
const double s450 = 450.0;
const double s600 = 600.0;

const double _tiny = s4;
const double _small = s8;
const double _medium = s16;
const double _large = s24;
const double _massive = s32;
const double _enormous = s40;

final SizedBox horizontalSpaceTiny = SizedBox(width: _tiny);
final SizedBox horizontalSpaceSmall = SizedBox(width: _small);
final SizedBox horizontalSpaceMedium = SizedBox(width: _medium);
final SizedBox horizontalSpaceLarge = SizedBox(width: _large);
final SizedBox horizontalSpaceMassive = SizedBox(width: _massive);
final SizedBox horizontalSpaceEnormous = SizedBox(width: _enormous);

final SizedBox verticalSpaceTiny = SizedBox(height: _tiny);
final SizedBox verticalSpaceSmall = SizedBox(height: _small);
final SizedBox verticalSpaceMedium = SizedBox(height: _medium);
final SizedBox verticalSpaceLarge = SizedBox(height: _large);
final SizedBox verticalSpaceMassive = SizedBox(height: _massive);
final SizedBox verticalSpaceEnormous = SizedBox(height: _enormous);
